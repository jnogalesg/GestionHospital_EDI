...
FechaYHora::FechaYHora (const string &fh) {
   ...
   this->mes     = atoi((mes.c_str()));
   this->anio    = atoi((anio.c_str()));
   this->hora    = atoi((hora.c_str()));
   this->minutos = atoi((minutos.c_str()));
}
...
string FechaYHora::toString() {
	string fecha;
	fecha = to_string(dia)+"/"+to_string(mes)+"/"+to_string(anio)+" "+to_string(hora)+":"+to_string(minutos);
